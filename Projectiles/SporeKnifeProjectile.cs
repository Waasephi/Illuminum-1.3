...
{
	public class SporeKnifeProjectile : ModProjectile
	{
		...
		{                                                        
			...
			{
				int dust = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, DustID.GrassBlades, projectile.velocity.X * 1.2f, projectile.velocity.Y * 1.2f, 1, default(Color), 3.50f);   //this make so when this projectile disappear will spawn dust, change PinkPlame to what dust you want from Terraria, or add mod.DustType("CustomDustName") for your custom dust
