			...
			if (projectile.frameCounter >= 60)
			{
				...
				if (num2 != -1)
				{
					...
					if (flag)
					{
						...
						value *= num5;
						Terraria.Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, value.X, value.Y, ProjectileID.IceBolt, projectile.damage, projectile.knockBack / 2f, projectile.owner, 0f, 0f);
					}
				}
			}
			return true;
