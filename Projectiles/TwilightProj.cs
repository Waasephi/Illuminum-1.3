...
{
	public class TwilightProj : ModProjectile
	{
			...
			if (projectile.ai[0] == 20)
            {
				...
				for (int i = 0; i < projectile.oldPos.Length; i++)
				{
					for (int k = 0; k < 8; k++)
                    {
						...
						dust = Main.dust[Terraria.Dust.NewDust(position, 0, 0, DustID.PurpleCrystalShard, 0f, 0f, 0, new Color(255, 255, 255), 0.828947f)];
						dust.noGravity = true;
					}
				}
			}
		...
		{
			if (projectile.ai[0] < 20) return Color.MediumPurple;
